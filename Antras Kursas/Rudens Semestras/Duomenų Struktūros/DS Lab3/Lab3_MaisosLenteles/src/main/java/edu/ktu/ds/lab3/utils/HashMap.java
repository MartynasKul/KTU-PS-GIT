...
public class HashMap<K, V> implements EvaluableMap<K, V> {
    ...
    public V remove(K key) {
        if (key == null) {
            throw new IllegalArgumentException("Null Key in remove(K key)!");
        }
        int indeksas = HashManager.hash(key.hashCode(), table.length, ht);
        Node<K, V> previous = null;

        for(Node<K, V> n = table[indeksas]; n!= null; n=n.next){
            if(n.key.equals(key)){
                if(previous == null){
                    table[indeksas] = n.next;
                }
                else{
                    previous.next = n.next;
                }
                size--; // sumazinamas aibes dydis

                if(table[indeksas] == null){
                    chainsCounter--;
                }
                return n.value;
            }
    ...
    public boolean replace(K key, V oldValue, V newValue) {
        int indeksas = HashManager.hash(key.hashCode(), table.length, ht);
        Node<K, V> keiciamas = getInChain(key, table[indeksas]);

        if(keiciamas != null && keiciamas.value.equals(oldValue)){
            keiciamas.value=newValue;
            return true;
        }
