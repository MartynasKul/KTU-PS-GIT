package scalatron.botwar.botPlugin.markul4
// Martynas Kulieðius IFF-1/9
        ...
        if( bot.energy > 0 && (bot.time % 30) == 0)
        {
            ...
            nearestEnemyMaster match {
                ...
                case Some(relPos) =>
                    ...
                    bot.spawn(unitDelta, "mood" -> "SnorgTempter", "target" -> remainder)
                    bot.say("I did it :D")
            }
        }
        
        if ((closestEmaster.stepCount < 4 || closestEnemy.stepCount < 1) && bot.energy > 100 )
        {
    ...
    def GetPath(view: View, find: Char, maxSearch: Int): XY = {
        ...
        while(!locationQueue.isEmpty && counter < maxSearch)
        {
            ...
            for (direction <- Seq(1,3,5,7,0,2,4,6))
            {
                ...
                {
                    val newLocationDirection = if(current.direction != -1) current.direction else direction
                    val newLocation = new Location(goingTo, current.distance + 1, newLocationDirection)
                    if(newLocation.distance < bestFind.distance && view.cellAtRelPos(goingTo) == find)
                    {
                        bestFind = newLocation
                    }
                    locationQueue.enqueue(new Location(goingTo, current.distance + 1, newLocationDirection))
                }            
            }      
        }
        
        if (bestFind.direction != -1) XY.fromDirection45(bestFind.direction) else new XY(0,0)
    }
}
// -------------------------------------------------------------------------------------------------
// Framework
